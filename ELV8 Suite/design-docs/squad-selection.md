...
/* FORMATION VIEW           */
/* ======================== */
.formation-container { display: flex; gap: 20px; align-items: flex-start; }
.formation-main { flex: 1; min-width: 0; }
...
/* Drop zones on field */
.field-drop { position: absolute; inset: 0; width: 100%; height: 100%; }

/* Player chip on field */
.field-player { position: absolute; transform: translate(-50%, -50%); cursor: grab; user-select: none; z-index: 10; transition: box-shadow .15s; }
.field-player:active { cursor: grabbing; z-index: 100; }
...
<nav>
  <span class="logo">SQUAD SELECTION</span>
  <button class="active" onclick="switchView('squad')">Squad</button>
  <button onclick="switchView('matches')">Matches</button>
  <button onclick="switchView('formation')">Formation</button>
  <button onclick="switchView('preview')">Preview / Print</button>
</nav>

<div id="view-squad" class="view active"></div>
<div id="view-matches" class="view"></div>
<div id="view-formation" class="view"></div>
<div id="view-preview" class="view"></div>

<script>
const STORAGE_KEY = 'hockeySquadSelection';
const POS_ORDER = ['GK','DEF','MID','STR'];
const POS_GROUPS = { GK: 'Goalkeepers', DEF: 'Defenders', MID: 'Midfielders', STR: 'Strikers' };
...
let dragState = { playerId: null, originZone: null };

function saveState() { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); }
function loadState() {
  const raw = localStorage.getItem(STORAGE_KEY);
  if (raw) { try { Object.assign(state, JSON.parse(raw)); } catch(e) {} }
  if (state.ui.currentQuarter === undefined) state.ui.currentQuarter = 0;
}
...
function switchView(v) {
  ...
  document.querySelectorAll('nav button').forEach(b => {
    ...
    if ((v==='squad' && t==='squad') || (v==='matches' && t==='matches') || (v==='formation' && t==='formation') || (v==='preview' && t.includes('preview'))) b.classList.add('active');
  });
  if (v==='squad') renderSquad();
  else if (v==='matches') renderMatches();
  else if (v==='formation') renderFormation();
  else if (v==='preview') renderPreview();
  saveState();
}

// =============================================
// FIELD SVG (hockey pitch markings)
// =============================================
function fieldSVG(idPrefix) {
  // Full hockey field, left = our goal, right = opposition
  return `<svg class="field-svg" viewBox="0 0 1000 690" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <!-- Left shooting circle (D) - large arc -->
    ...
    <!-- Right shooting circle (D) - large arc -->
...
function renderSquad() {
  ...
  let html = `<h2>Wider Squad</h2>
    <div class="toolbar">
      <button class="btn btn-primary" onclick="addPlayer()">+ Add Player</button>
      <button class="btn btn-secondary" onclick="addMultiplePlayers()">+ Bulk Add</button>
    ...
    <div class="section">
      <table class="squad-table">
        <thead><tr>
          ...
          <th style="width:80px;">Caps</th>
          <th style="width:40px;"></th>
        </tr></thead>
        <tbody>`;

  const sorted = [...state.squad].sort((a,b) => {
    const ai = POS_ORDER.indexOf(a.posGroup || 'STR');
    const bi = POS_ORDER.indexOf(b.posGroup || 'STR');
  ...
  sorted.forEach((p, si) => {
    ...
    html += `<tr>
      <td style="cursor:pointer;" onclick="uploadPhotoById('${p.id}')">${p.photo ? `<img src="${esc(p.photo)}">` : `<span class="initials">${initials}</span>`}</td>
      <td><input value="${esc(p.jersey)}" onchange="updatePlayer('${p.id}','jersey',this.value)" style="width:50px; text-align:center; font-weight:700;"></td>
      <td><input value="${esc(p.name)}" onchange="updatePlayer('${p.id}','name',this.value)" style="font-weight:600;"></td>
      <td><select onchange="updatePlayer('${p.id}','posGroup',this.value);renderSquad();">
        ${POS_ORDER.map(g => `<option value="${g}" ${p.posGroup===g?'selected':''}>${POS_GROUPS[g]}</option>`).join('')}
      </select></td>
      <td><input value="${esc(p.caps)}" onchange="updatePlayer('${p.id}','caps',this.value)" style="width:60px; text-align:center;"></td>
      <td><button class="btn btn-danger btn-sm" onclick="removePlayerById('${p.id}')">×</button></td>
    </tr>`;
  });
...
function addPlayer() {
  state.squad.push({ id: uid(), name: '', jersey: '', posGroup: 'MID', caps: '', photo: '' });
  saveState(); renderSquad();
  setTimeout(() => {
    const inputs = document.querySelectorAll('.squad-table td input');
    if (inputs.length > 0) inputs[inputs.length - 4].focus();
  }, 50);
}

function addMultiplePlayers() {
  const text = prompt('Paste player names (one per line):');
  if (!text) return;
  text.split('\n').forEach(line => {
    const name = line.trim();
    if (name) state.squad.push({ id: uid(), name, jersey: '', posGroup: 'MID', caps: '', photo: '' });
  });
  saveState(); renderSquad();
}
...
function removePlayerById(id) {
  ...
  state.squad = state.squad.filter(pl => pl.id !== id);
  saveState(); renderSquad();
}

function uploadPhotoById(id) {
  const p = state.squad.find(pl => pl.id === id);
  if (!p) return;
  ...
  input.onchange = e => {
    ...
    reader.onload = ev => {
      img.onload = () => {
        ...
        p.photo = canvas.toDataURL('image/jpeg', 0.7);
        saveState(); renderSquad();
      };
...
function exportData() {
  ...
  const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'squad-selection-data.json'; a.click();
}
  ...
  input.onchange = e => {
    ...
    reader.onload = ev => {
      try {
        ...
        if (imported.squad) { state.squad = imported.squad; state.matches = imported.matches || []; }
        else if (Array.isArray(imported)) { state.squad = [...state.squad, ...imported]; }
        saveState(); renderSquad();
...
function renderMatches() {
  ...
  if (state.ui.currentMatchId) { renderSelection(); return; }

  let html = `<h2>Match Selections</h2>
    <div class="toolbar">
      <button class="btn btn-primary" onclick="createMatch()">+ New Match</button>
    </div>`;

  state.matches.forEach(m => {
    const selected = (m.selected || []).length;
    html += `<div class="match-card" onclick="openMatch('${m.id}')">
      <div class="info"><h4>${esc(m.title)}</h4><small>${esc(m.date)} · ${esc(m.venue)} · ${selected} selected</small></div>
      <div style="display:flex;gap:4px;">
        <button class="btn btn-secondary btn-sm" onclick="event.stopPropagation();dupMatch('${m.id}')">Dup</button>
        <button class="btn btn-danger btn-sm" onclick="event.stopPropagation();delMatch('${m.id}')">×</button>
      </div>
    </div>`;
  });
...
function createMatch() {
  ...
  const m = {
    ...
    selected: [],
    roles: {},
    formations: [{},{},{},{}] // per-quarter: { playerId: { x, y, zone } }
  };
  state.matches.push(m);
  state.ui.currentMatchId = m.id;
  saveState(); renderSelection();
}

function openMatch(id) { state.ui.currentMatchId = id; saveState(); renderSelection(); }
function dupMatch(id) {
  const orig = state.matches.find(m=>m.id===id); if (!orig) return;
  const copy = JSON.parse(JSON.stringify(orig)); copy.id = uid(); copy.title += ' (copy)';
  state.matches.push(copy); saveState(); renderMatches();
}
function delMatch(id) {
  if (!confirm('Delete?')) return;
  state.matches = state.matches.filter(m=>m.id!==id);
  if (state.ui.currentMatchId===id) state.ui.currentMatchId = null;
  saveState(); renderMatches();
}

function getMatch() { return state.matches.find(m=>m.id===state.ui.currentMatchId); }

function renderSelection() {
  const el = document.getElementById('view-matches');
  const match = getMatch();
  if (!match) { renderMatches(); return; }

  const selected = new Set(match.selected || []);
  const roles = match.roles || {};

  let html = `<div class="toolbar">
    <button class="btn btn-secondary btn-sm" onclick="state.ui.currentMatchId=null;saveState();renderMatches();">← Back</button>
    <button class="btn btn-success btn-sm" onclick="switchView('formation')">Formation →</button>
    <button class="btn btn-success btn-sm" onclick="switchView('preview')">Preview / Print →</button>
  </div>
  <div class="section">
    <h3>Match Details</h3>
    <div class="form-row">
      <div class="form-group" style="flex:2;"><label>Match Title</label><input value="${esc(match.title)}" onchange="getMatch().title=this.value;saveState();"></div>
      <div class="form-group"><label>Date</label><input value="${esc(match.date)}" onchange="getMatch().date=this.value;saveState();"></div>
      <div class="form-group"><label>Time</label><input value="${esc(match.time)}" onchange="getMatch().time=this.value;saveState();" placeholder="19:00"></div>
    </div>
    <div class="form-row">
      <div class="form-group" style="flex:1;"><label>Venue</label><input value="${esc(match.venue)}" onchange="getMatch().venue=this.value;saveState();"></div>
      <div class="form-group" style="flex:1;"><label>Competition</label><input value="${esc(match.competition)}" onchange="getMatch().competition=this.value;saveState();" placeholder="Pro League"></div>
      <div class="form-group" style="flex:1;"><label>Team Name</label><input value="${esc(match.teamName)}" onchange="getMatch().teamName=this.value;saveState();"></div>
    </div>
    <div class="form-row">
      <div class="form-group" style="flex:1;"><label>Notes (captain, vice-captain, etc.)</label><textarea rows="2" onchange="getMatch().notes=this.value;saveState();" style="width:100%;">${esc(match.notes)}</textarea></div>
    </div>
  </div>`;

  const starters = [...selected].filter(id => roles[id] === 'starter' || !roles[id]);
  const interchange = [...selected].filter(id => roles[id] === 'interchange');
  const starterCount = selected.size - interchange.length;

  html += `<div class="section">
    <h3>Select Players</h3>
    <div class="selection-count">Selected: <span>${selected.size}</span> / ${state.squad.length} &nbsp;|&nbsp; Starting: <span>${starterCount}</span> &nbsp;|&nbsp; Interchange: <span>${interchange.length}</span></div>`;

  POS_ORDER.forEach(group => {
    const players = state.squad.filter(p => p.posGroup === group);
    if (players.length === 0) return;
    html += `<div class="selection-group"><h4>${POS_GROUPS[group]}</h4>`;
    players.forEach(p => {
      const isSel = selected.has(p.id);
      const role = roles[p.id] || 'starter';
      html += `<div class="selection-row ${isSel?'selected':''}" onclick="toggleSelect('${p.id}')">
        <div class="sel-check">${isSel ? '✓' : ''}</div>
        <div class="sel-jersey">${esc(p.jersey)}</div>
        <div class="sel-name">${esc(p.name)}</div>
        <div class="sel-pos">${esc(POS_GROUPS[p.posGroup])}</div>
        ${isSel ? `<div class="sel-role" onclick="event.stopPropagation();">
          <select onchange="getMatch().roles['${p.id}']=this.value;saveState();renderSelection();">
            <option value="starter" ${role==='starter'?'selected':''}>Starter</option>
            <option value="interchange" ${role==='interchange'?'selected':''}>Interchange</option>
          </select>
        </div>` : ''}
      </div>`;
    });
...
function toggleSelect(playerId) {
  ...
  if (!match.roles) match.roles = {};
  const idx = match.selected.indexOf(playerId);
  if (idx >= 0) {
    match.selected.splice(idx, 1);
    delete match.roles[playerId];
  } else {
    match.selected.push(playerId);
    match.roles[playerId] = 'starter';
  }
  saveState(); renderSelection();
}
...
// FORMATION VIEW
// =============================================
function ensureFormations(match) {
  if (!match.formations) match.formations = [{},{},{},{}];
  while (match.formations.length < 4) match.formations.push({});
}

function getQuarterData(match, q) {
  ensureFormations(match);
  return match.formations[q] || {};
}

function getFormationCounts(match, q) {
  const qd = getQuarterData(match, q);
  const selected = match.selected || [];
  let field = 0, bench = 0, injured = 0;
  selected.forEach(pid => {
    const pd = qd[pid];
    if (pd && pd.zone === 'injured') injured++;
    else if (pd && pd.zone === 'bench') bench++;
    else if (pd && pd.zone === 'field') field++;
    else {
      // Auto-assign based on role
      const role = (match.roles || {})[pid];
      if (role === 'interchange') bench++;
      else field++;
    }
  });
  return { field, bench, injured };
}

function getFormationString(match, q) {
  const qd = getQuarterData(match, q);
  const selected = match.selected || [];
  const roles = match.roles || {};
  // Count field players by position group
  const counts = { DEF: 0, MID: 0, STR: 0 };
  selected.forEach(pid => {
    const pd = qd[pid];
    const zone = pd ? pd.zone : (roles[pid] === 'interchange' ? 'bench' : 'field');
    if (zone === 'field') {
      const player = state.squad.find(p => p.id === pid);
      if (player && player.posGroup !== 'GK') {
        counts[player.posGroup] = (counts[player.posGroup] || 0) + 1;
      }
    }
  });
  return `${counts.DEF}-${counts.MID}-${counts.STR}`;
}

function renderFormation() {
  const el = document.getElementById('view-formation');
  const match = getMatch();
  ...
  if (!match) {
    ...
    if (state.matches.length === 0) {
      ...
      html += `<div class="toolbar"><select id="formation-match-select" onchange="selectFormationMatch(this.value)" style="padding:8px 12px;">
        ...
        ${state.matches.map(m => `<option value="${m.id}">${esc(m.title)} - ${esc(m.date)}</option>`).join('')}
      </select></div>`;
    }
    el.innerHTML = html;
    return;
  ...
  ensureFormations(match);
  const q = state.ui.currentQuarter || 0;
  const qd = getQuarterData(match, q);
  const selected = match.selected || [];
  const roles = match.roles || {};
  const formStr = getFormationString(match, q);
  const counts = getFormationCounts(match, q);

  // Categorize players for this quarter
  const fieldPlayers = [];
  const benchPlayers = [];
  ...
  selected.forEach(pid => {
    ...
    if (!player) return;
    const pd = qd[pid];
    if (pd && pd.zone === 'field') fieldPlayers.push({ ...player, x: pd.x, y: pd.y });
    else if (pd && pd.zone === 'bench') benchPlayers.push(player);
    else if (pd && pd.zone === 'injured') injuredPlayers.push(player);
  ...
  // Quarter tabs
  html += `<div class="quarter-tabs">`;
  for (let i = 0; i < 4; i++) {
    html += `<button class="quarter-tab ${i===q?'active':''}" onclick="state.ui.currentQuarter=${i};saveState();renderFormation();">${QUARTER_LABELS[i]}</button>`;
  }
  html += `</div>`;

  html += `<div class="toolbar" style="margin-bottom:8px;">
    <div class="formation-label">${QUARTER_LABELS[q]} = ${formStr}</div>
    <div style="margin-left:auto; display:flex; gap:6px; align-items:center;">
      <button class="btn btn-secondary btn-sm" onclick="autoPlaceAll()">Auto-Place</button>
      <div class="copy-quarter-menu">
        <button class="btn btn-secondary btn-sm" onclick="toggleCopyMenu(event)">Copy From...</button>
        <div class="copy-quarter-dropdown" id="copy-quarter-dd">
          ${[0,1,2,3].filter(i=>i!==q).map(i => `<button onclick="copyQuarter(${i},${q});this.parentElement.classList.remove('show');">← ${QUARTER_LABELS[i]}</button>`).join('')}
        </div>
      </div>
      <button class="btn btn-danger btn-sm" onclick="clearQuarter(${q})">Clear</button>
    </div>
  </div>`;

  html += `<div class="formation-container">
    <div class="formation-main">`;

  // Injured bar (top)
  html += `<div class="injured-bar" id="injured-drop" ondragover="allowDrop(event)" ondrop="dropToZone(event,'injured')">`;
  if (injuredPlayers.length === 0) html += `<span class="bar-empty">INJURED — drag players here</span>`;
  injuredPlayers.forEach(p => {
    const surname = getDisplayName(p);
    html += `<span class="fp-label" draggable="true" ondragstart="startDrag(event,'${p.id}','injured')">${esc(surname)}</span>`;
  });
  html += `</div>`;

  // Field
  html += `<div class="field-wrapper" id="field-drop" ondragover="allowDrop(event)" ondrop="dropToField(event)">
    ${fieldSVG('fm')}
    <div class="field-drop">`;

  fieldPlayers.forEach(p => {
    const surname = getDisplayName(p);
    html += `<div class="field-player" style="left:${p.x}%;top:${p.y}%;" draggable="true" ondragstart="startDrag(event,'${p.id}','field')" id="fp-${p.id}">
      <div class="fp-label">${esc(surname)}</div>
    </div>`;
  });
  ...
  // Bench bar (bottom)
  html += `<div class="bench-bar" id="bench-drop" ondragover="allowDrop(event)" ondrop="dropToZone(event,'bench')">`;
  if (benchPlayers.length === 0 && unassigned.length === 0) html += `<span class="bar-empty">BENCH — drag players here</span>`;
  benchPlayers.forEach(p => {
    const surname = getDisplayName(p);
    html += `<span class="fp-label" draggable="true" ondragstart="startDrag(event,'${p.id}','bench')">${esc(surname)}</span>`;
  });
  html += `</div>`;

  html += `</div>`; // end formation-main
  ...
  POS_ORDER.forEach(group => {
    const players = state.squad.filter(p => p.posGroup === group && selected.includes(p.id));
    if (players.length === 0) return;
    html += `<div class="pool-group-title">${POS_GROUPS[group]}</div>`;
    players.forEach(p => {
      const pd = qd[p.id];
      let statusClass = 'unassigned';
      if (pd) {
        if (pd.zone === 'field') statusClass = 'on-field';
        else if (pd.zone === 'bench') statusClass = 'on-bench';
        else if (pd.zone === 'injured') statusClass = 'is-injured';
      }
      const surname = getDisplayName(p);
      html += `<div class="pool-player" draggable="true" ondragstart="startDrag(event,'${p.id}','pool')">
        <span class="pp-jersey">${esc(p.jersey)}</span>
        <span class="pp-name">${esc(surname)}</span>
        <span class="pp-status ${statusClass}"></span>
      </div>`;
    });
  });
  ...
  html += `</div></div>`; // end sidebar + container
...
function getDisplayName(player) {
  ...
  // Use last name, but if there are duplicates we need initial
  const lastName = parts[parts.length - 1];
  const dupes = state.squad.filter(p => {
    const pp = (p.name || '').trim().split(' ');
    return pp.length > 0 && pp[pp.length - 1] === lastName && p.id !== player.id;
  });
  if (dupes.length > 0) {
...
function selectFormationMatch(id) {
  if (!id) return;
  state.ui.currentMatchId = id;
  saveState();
  renderFormation();
}

// Drag and drop
function startDrag(e, playerId, originZone) {
  dragState.playerId = playerId;
  dragState.originZone = originZone;
...
function dropToField(e) {
  e.preventDefault();
  const match = getMatch();
  if (!match || !dragState.playerId) return;
  const q = state.ui.currentQuarter || 0;
  ensureFormations(match);
  ...
  // Clamp
  const cx = Math.max(2, Math.min(98, parseFloat(x)));
  const cy = Math.max(3, Math.min(97, parseFloat(y)));

  match.formations[q][dragState.playerId] = { x: cx, y: cy, zone: 'field' };
...
function dropToZone(e, zone) {
  ...
  match.formations[q][dragState.playerId] = { x: 50, y: 50, zone: zone };
  dragState.playerId = null;
  saveState();
  renderFormation();
}
...
function toggleCopyMenu(e) {
  e.stopPropagation();
  const dd = document.getElementById('copy-quarter-dd');
...
function copyQuarter(from, to) {
  const match = getMatch(); if (!match) return;
  ensureFormations(match);
  match.formations[to] = JSON.parse(JSON.stringify(match.formations[from] || {}));
  saveState();
  renderFormation();
}

function clearQuarter(q) {
  const match = getMatch(); if (!match) return;
  if (!confirm('Clear all positions for ' + QUARTER_LABELS[q] + '?')) return;
  ensureFormations(match);
  match.formations[q] = {};
  saveState();
  renderFormation();
...
function autoPlaceAll() {
  ...
  const selected = match.selected || [];
  const roles = match.roles || {};
  const qd = match.formations[q];

  // Place starters on field, interchange on bench
  // Auto-position by position group
  const starters = selected.filter(pid => roles[pid] !== 'interchange');
  const interchangers = selected.filter(pid => roles[pid] === 'interchange');

  // Group starters by position
  const groups = { GK: [], DEF: [], MID: [], STR: [] };
  starters.forEach(pid => {
    const p = state.squad.find(pl => pl.id === pid);
    if (p) groups[p.posGroup || 'MID'].push(pid);
  });

  // Auto-position: GK near left, DEF left-center, MID center, STR right-center
  const xZones = { GK: 6, DEF: 25, MID: 45, STR: 65 };

  Object.keys(groups).forEach(g => {
    const pids = groups[g];
    const count = pids.length;
    const baseX = xZones[g];
    pids.forEach((pid, i) => {
      // Spread vertically
      const ySpacing = 80 / (count + 1);
      const y = 10 + ySpacing * (i + 1);
      // Slight x variation for visual interest
      const xOff = count > 1 ? (i % 2 === 0 ? -4 : 4) : 0;
      // Keep existing position if already on field
      if (qd[pid] && qd[pid].zone === 'field') return;
      qd[pid] = { x: baseX + xOff, y: y, zone: 'field' };
    });
  });

  // Place interchange on bench
  interchangers.forEach(pid => {
    if (qd[pid] && qd[pid].zone === 'bench') return;
    qd[pid] = { x: 50, y: 50, zone: 'bench' };
  });
...
function setupTouchDrag() {
  ...
  let touchGhost = null;
  let touchPlayerId = null;

  document.querySelectorAll('[draggable="true"]').forEach(el => {
    el.addEventListener('touchstart', function(e) {
      const dragstartEvent = el.getAttribute('ondragstart');
      if (!dragstartEvent) return;
      const match = dragstartEvent.match(/startDrag\(event,'([^']+)','([^']+)'\)/);
      if (!match) return;

      touchPlayerId = match[1];
      dragState.playerId = touchPlayerId;
      dragState.originZone = match[2];
  ...
  document.addEventListener('touchmove', function(e) {
    ...
    touchGhost.style.top = (touch.clientY - 15) + 'px';
    e.preventDefault();
  }, { passive: false });

  document.addEventListener('touchend', function(e) {
    if (!touchGhost || !touchPlayerId) return;
    ...
    const benchEl = document.getElementById('bench-drop');
    const injuredEl = document.getElementById('injured-drop');

    const dropTarget = document.elementFromPoint(touch.clientX, touch.clientY);

    if (fieldEl && (fieldEl.contains(dropTarget) || dropTarget === fieldEl)) {
      const rect = fieldEl.getBoundingClientRect();
      const fakeEvent = { preventDefault: ()=>{}, clientX: touch.clientX, clientY: touch.clientY };
      dropToField(fakeEvent);
    } else if (benchEl && (benchEl.contains(dropTarget) || dropTarget === benchEl)) {
      const fakeEvent = { preventDefault: ()=>{} };
      dropToZone(fakeEvent, 'bench');
    } else if (injuredEl && (injuredEl.contains(dropTarget) || dropTarget === injuredEl)) {
      const fakeEvent = { preventDefault: ()=>{} };
      dropToZone(fakeEvent, 'injured');
    }
...
function renderPreview() {
  ...
  let html = `<div class="no-print" style="margin-bottom:16px; display:flex; gap:8px; align-items:center;">
    ...
    <label style="display:flex; align-items:center; gap:4px; margin:0; cursor:pointer;">
      <input type="checkbox" id="preview-formation" checked onchange="renderPreviewContent()" style="width:auto;"> Formation
    </label>
    <label style="display:flex; align-items:center; gap:4px; margin:0; cursor:pointer;">
...
function renderPreviewContent() {
  ...
  if (showFormation) {
    ...
    for (let q = 0; q < 4; q++) {
      ...
      selected.forEach(pid => {
        ...
        if (!player) return;
        const pd = qd[pid];
        if (pd && pd.zone === 'field') fieldPs.push({ ...player, x: pd.x, y: pd.y });
        else if (pd && pd.zone === 'bench') benchPs.push(player);
        else if (pd && pd.zone === 'injured') injuredPs.push(player);
        else if (roles[pid] === 'interchange') benchPs.push(player);
      ...
      fieldPs.forEach(p => {
        html += `<div class="fo-player" style="left:${p.x}%;top:${p.y}%;">
          <div class="fo-label">${esc(getDisplayName(p))}</div>
        </div>`;
      });
  ...
  if (showTeamSheet) {
    const roles = match.roles || {};
    const selectedPlayers = (match.selected || []).map(id => state.squad.find(p => p.id === id)).filter(Boolean);
    const starters = selectedPlayers.filter(p => roles[p.id] !== 'interchange');
    const interchange = selectedPlayers.filter(p => roles[p.id] === 'interchange');
    ...
    POS_ORDER.forEach(group => {
      ...
      players.forEach(p => {
        html += `<tr data-pos="${esc(p.posGroup)}">
          ...
          <td class="cap-num">${esc(p.caps)}</td>
        </tr>`;
      });
    });
